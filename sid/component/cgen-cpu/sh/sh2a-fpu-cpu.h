/* CPU class elements for sh2a_fpu.
...
// CPU state information.
  ...
  {
    for (int i = 0; i < sh2a_fpu::pipe_sz; i++)
    {
      ost << st[i].t << ' ';
  ...
  void stream_cgen_write_stacks (std::ostream &ost, const sh2a_fpu::write_stacks &stacks) const 
  {
    stream_stacks ( stacks.h_pc_writes, ost);
    stream_stacks ( stacks.h_pr_writes, ost);
  }
  void destream_cgen_write_stacks (std::istream &ist, sh2a_fpu::write_stacks &stacks) 
  {
