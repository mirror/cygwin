/* CPU class elements for sh4a_nofpu.
...
// CPU state information.
  ...
  {
    for (int i = 0; i < sh4a_nofpu::pipe_sz; i++)
    {
      ost << st[i].t << ' ';
  ...
  void stream_cgen_write_stacks (std::ostream &ost, const sh4a_nofpu::write_stacks &stacks) const 
  {
    stream_stacks ( stacks.h_pc_writes, ost);
    stream_stacks ( stacks.h_pr_writes, ost);
  }
  void destream_cgen_write_stacks (std::istream &ist, sh4a_nofpu::write_stacks &stacks) 
  {
